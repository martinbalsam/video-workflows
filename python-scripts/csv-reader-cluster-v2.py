...
#initialize all the lists for the values in the csv file
videoid_list =[]
amount_spent_list = []
ctr_list = []
awt_list = []
...
with open(csvfilename) as csvfile:
    ...
    for row in reader:
        ...
        #handling errors if the csv file does not contain all the following entries
        try:
...
#TODO: refact code to just simply use .index() method
def id_to_index(id):
...
def get_list_nx4(l):
    count = 0
    #counts all the videos in the list and put them in a separate list
    video_list =[]
    for file in l:
    ...
    n = 4
    return [video_list[i * n:(i + 1) * n] for i in range((len(video_list) + n - 1) // n )]  


subdivided_video_list = get_list_nx4(sorted(os.listdir(dirpath)))

print subdivided_video_list


for row in subdivided_video_list:
    html_body+="<table>"
    ...
    for sub_row in table_html_list:
        html_body+=sub_row+"</tr>"
